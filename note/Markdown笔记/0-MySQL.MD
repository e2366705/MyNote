...
* 同财务相关的金额类数据必须使用decimal类型 ( decimal类型是MySQL官方唯一指定能精确存储的类型)
	非精准浮点：float,double
	精准浮点：decimal
		...
		数据库小数不要使用double了，除非强制要求！小数一律使用decimal(numeric )。这两个类型是一样的
...
<details>
<summary><b> 备份_1: BackUP_MySQL.bat (Windows批处理)</b></summary>
...
<details>
<summary><b> 备份_2: BackUP_to_github.bat (Windows批处理)</b></summary>
...
@forfiles /p "D:\MySQL_BackUp" /m backup_*.sql -d -30 /c "cmd /c del /f @path"
@set "time=%date:~0,4%-%date:~5,2%-%date:~8,2%-%time:~1,1%%time:~3,2%%time:~6,2%"
@"C:\phpStudy\PHPTutorial\MySQL\bin\mysqldump" --opt --single-transaction=TRUE --user=root --password=root --host=127.0.0.1 --protocol=tcp --port=3306 --default-character-set=utf8 --single-transaction=TRUE --routines --events "study_english_database" > "C:\D\Nut_cloud___\我的坚果云\笔记\Github_Notes\MyNote\note\Markdown笔记\MySQL_backup\study_english_database_%p%%d%__%t%.sql"
...
升序 / 降序 查询:
升序 ascending(asc):     从小到大的排列 (数字: 0 - 100 , 字母: A - Z)
降序 descending(desc):   反之(从大到小)
	降序
