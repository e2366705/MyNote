...
<details>
<summary><b> 逻辑运算 (and / or) </b></summary>
...
字符串时间 -> 时间戳
import time
tss1 = '2019-06-03 21:19:03'                                  # 字符类型的时间
timeArray = time.strptime(tss1, "%Y-%m-%d %H:%M:%S")          # 转为时间数组
print(timeArray)                                              # time.struct_time(tm_year=2019, tm_mon=6, tm_mday=3, tm_hour=21, tm_min=19, tm_sec=3, tm_wday=0, tm_yday=154, tm_isdst=-1)
...
print(otherStyleTime)                                          #  2019-06-03 21:19:03
...
languages = ["C", "C++", "Perl", "Python"]
for index, val in enumerate(languages):
    print(index , val)
输出:
    0 C
    1 C++
...
也可以使range以指定数字开始并指定不同的增量(甚至可以是负数，有时这也叫做'步长', 这个循环中, 25就是步长):
...
# 字符串
<details>
<summary><b> 字符串: 格式化/截取/替换/分割/ </b></summary>
...
-----------------------------------   格式化字符串   ----------------------------------- 
Python提倡用一种，而且最好是只有一种方法来完成一件事。
可是在字符串格式化方面Python好像没有做到这一点。
除了基本的%-formatting格式，还有str.format()格式、以及string.Template
...
name = "Tom"
age = 3
aaa = f"His name is {name}, he's {age} years old"
print(aaa)               # His name is Tom, he's 3 years old
...
aaa = " %f " % (100.666)
print(aaa)                #  100.666000 


spurs = {"AAA": "aaa", "BBB": "bbb"}
aaa = f"  {len(spurs)}     ---     {spurs['AAA']}        ---      {spurs['BBB']}        ---     "
print(aaa)
输出:
      2     ---     aaa        ---      bbb        ---     
...
把所有的中文, 替换成空字符串:
import re
print(re.sub(r'[\u4e00-\u9fa5]+' , '', "ASDaaa_为什么1231_qweqwQWEQWE"))
输出:
    ASDaaa_1231_qweqwQWEQWE
...
print("英语四级阅读理解100篇精讲之论说“要求政府禁烟”".split("之") )
输出:
    ['英语四级阅读理解100篇精讲', '论说“要求政府禁烟”']
...
# 中文匹配正则
import re
TXT = "啊啊 12312 ASDAS asdasas哈哈哈dasdas哦dasd"
chinese_pattern = '[\u4e00-\u9fa5]+'
list_ch = re.findall(chinese_pattern, TXT)        # 返回一个列表
print(list_ch)
输出:
    ['啊啊', '哈哈哈', '哦']
...
# 从 1-100 中随机选取一个数, 变化幅度至少为 10
print ("randrange(1,100, 2) : ", random.randrange(1, 100, 10))

# 从 1-100 中随机选取一个数, 变化幅度至少为 2
...
总结:
    ...
    Set(集合):              无序的 , 不重复的, 可以使用大括号 { } 或者 set() 函数创建集合，
...
-----------------------------------   list & string   ------------------------------------
--- list => string:
list_test = ['AAAAAAAA', 'bbbbbbbbbb', 'CCCCCCCCccccccc']
to_str = "-".join(list_test)
print(to_str)            # AAAAAAAA-bbbbbbbbbb-CCCCCCCCccccccc

list1=[1,'two','three',4]
print(" ".join('%s' %id for id in list1))     # 1 two three 4
...
-----------------------------------   tuple => string:    ----------------------------------- 
value = ('www.runoob.com', 14)                # <class 'tuple'>
s = str(value)                                # <class 'str'>
...
print("Aaaaaaa  "+format(100))         # 输出 : Aaaaaaa  100
print("Aaaaaaa  "+ str(100))           # 输出 : Aaaaaaa  100
print("Aaaaaaa  "+ (100))              # 会报错
...
输出:
	{'title': '范特西', 'star': '9.2', 'brief_introduction': '周杰伦 / 2001-09-14 / 专辑 / CD / 流行'}
	<class 'dict'>
...
逐行读取:
import os
for line in open(r"C:\Users\TEST.txt" , encoding="UTF-8"):
    if line != "\n":
        line = line.replace("\n", "")         # 把 换行符替换掉
        print(line)
...
print(FilePath)
输出:
    C:\Users\SpringBoot\PycharmProjects\untitled1\pictureJay\



import os
path = r"C:\Users\SpringBoot\PycharmProjects\untitled1\English_Article_Spider\English_Article"  
files = os.listdir(path)                    # 得到所有文件名称
for file in files:                          # 遍历
    if not os.path.isdir(file):             # 判断是否是文件夹，不是文件夹才打开
        print("文件名称为:   " + file )     # 输出 当前这个文件的文件名
...
详细资料 : https://www.runoob.com/python3/python3-json.html
...
# dumps:把字典转换为 json字符串
json_str = json.dumps(data)
print(json_str)               # {"statusCode": 200, "data": {"totoal": "5", "height": "5.97", "weight": "10.30", "age": "11"}, "msg": "\u6210\u529f"}
print(type(json_str))         # <class 'str'>

# loads:把json转换为 dict
json_dict = json.loads(json_str)
print(json_dict)             # {'statusCode': 200, 'data': {'totoal': '5', 'height': '5.97', 'weight': '10.30', 'age': '11'}, 'msg': '成功'}
print(type(json_dict))       # <class 'dict'>

# 打印statusCode对应的值 
print(json_dict["data"]["age"])   # 11

# 表示 subjects 下面的第一个项目, 下面的 title 的值
print(json_dict["subjects"][0]['title'])
...
pymysql.escape_string: 有时候有双引号/单引号在里面, 需要格式化转义一下
...
def insert(line_num, english, chinese, movie_name):
    cursor = db.cursor()                                     # 使用 cursor() 方法创建一个游标对象 cursor
    sql = '          INSERT INTO `tb_movie_script` VALUES(  "%s" , "%s" , "%s" , "%s" )        ' % (line_num, (pymysql.escape_string(english)), (pymysql.escape_string(chinese)), movie_name)
    print(sql)    
    --> pymysql.escape_string: 有时候有双引号/单引号在里面, 需要格式化转义一下
    try:
        ...
        print("insert Error   应该是数据重复??? ")
        db.rollback()                                        # 如果发生错误则回滚
    cursor.close()                                           # 关闭数据库连接

insert(0,  'Bubba " s'  ,  '哈哈哈哈哈哈'  ,  '阿甘正传')
db.close()


注意:   UPDATE  tb_sentence  SET time_stamp = %s  where time_stamp = \'\' LIMIT 1;  
        这里面的 where time_stamp = ''  一定要转义, 不然拼接到语句里面就变成 :
        UPDATE  tb_sentence  SET time_stamp = "1587889918244"  where time_stamp =  LIMIT 1;
        转义成为 : where time_stamp = \'\' LIMIT 1;  
        即可~
...
方式 2:
    ...
    r = con.content
    print(r.decode('utf-8'))          # 爬取出来的网页 转码, 转成 UTF-8 格式
...
<details>
<summary><b> 获取网页源码 - requests 方式  </b></summary>
...
<details>
<summary><b> Xpath : 基础 </b></summary>

```  

在 Chrome 浏览器中(Firefox也可以), 选择你要选择的内容的节点, 然后鼠标右键, 选择 Copy -> Copy Xpath , 即可复制 Xpath 路径:
...
def Local_html(URL):
    ...
    list = html.xpath('//html/body/article/div')
    print(list)
    for item in list:
        print(item.xpath(".//h1/text()"))   # 分类完，遍历时在调用xpath方法时,在路径前面加" . "直接指向当前路径，就不会指向顶层xpath路径

Local_html("./TEST.html")
...
# 找出 class='entry_tit' 的 a标签的 text (返回类型: 列表)
...
# 找出 class='entry_tit' 的 a标签的 链接 (返回类型: 列表)
...
# 找出 class 为 item 的 tr (返回类型: 列表)
count = html.xpath("//tr[@class='item']")


normalize-space:
    这个方法就可以去除文本中的前后空格和回车:
    title = info.xpath("normalize-space(td[2]/div/a/text())")             # 获取标题

    如果不加 normalize-space , 那么爬出来的数据就是这样子的:
    ['\n            We Sing. We Dance. We Steal Things.\n       ']

    如果加了 normalize-space , 那么爬取出来的数据是这样子的:
...
<details>
<summary><b> 解析本地文件 by:xpath </b></summary>
...
def Local_html(URL):
    html = etree.parse(URL, etree.HTMLParser())
    list = html.xpath('//*[@class="content-city"]')
    for index, item in enumerate(list):
        city = item.xpath(".//text()")
...
<details>
<summary><b> Xpath : 小案例 Demo </b></summary>

```

-----------------------------------   案例 1 : 爬取 豆瓣音乐 Top 250   -----------------------------------
...
def getResult():
   ...
   for url in  urls:
       ...
       html = etree.HTML(data.text)                                    # 格式化 html 格式
       #循环标签
       count = html.xpath("//tr[@class='item']")
       for info in count:
...
输出: 
    {'title': 'We Sing. We Dance. We Steal Things.', 'star': '9.1', 'brief_introduction': 'Jason Mraz / 2008-05-13 / Import / Audio CD / 民谣'}
    <class 'dict'>
    {'title': 'Viva La Vida', 'star': '8.7', 'brief_introduction': 'Coldplay / 2008-06-17 / 专辑 / CD / 摇滚'}
    <class 'dict'>
    {'title': '华丽的冒险', 'star': '8.9', 'brief_introduction': '陈绮贞 / 2005-09-23 / 专辑 / CD / 流行'}
    <class 'dict'>
    {'title': '范特西', 'star': '9.2', 'brief_introduction': '周杰伦 / 2001-09-14 / 专辑 / CD / 流行'}
    <class 'dict'>   
...
输出:
    肖申克的救赎 导演:弗兰克·德拉邦特FrankDarabont   主演:蒂姆·罗宾斯TimRobbins/... 9.7 1994   美国   犯罪剧情 1931647人评价
    霸王别姬 导演:陈凯歌KaigeChen   主演:张国荣LeslieCheung/张丰毅FengyiZha... 9.7 1993   中国大陆中国香港   剧情爱情同性 1931647人评价
    阿甘正传 导演:罗伯特·泽米吉斯RobertZemeckis   主演:汤姆·汉克斯TomHanks/... 9.7 1994   美国   剧情爱情 1931647人评价
    这个杀手不太冷 导演:吕克·贝松LucBesson   主演:让·雷诺JeanReno/娜塔莉·波特曼... 9.7 1994   法国   剧情动作犯罪 1931647人评价
    美丽人生 导演:罗伯托·贝尼尼RobertoBenigni   主演:罗伯托·贝尼尼RobertoBeni... 9.7 1997   意大利   剧情喜剧爱情战争 1931647人评价
    泰坦尼克号 导演:詹姆斯·卡梅隆JamesCameron   主演:莱昂纳多·迪卡普里奥Leonardo... 9.7 1997   美国   剧情爱情灾难 1931647人评价
...
<details>
<summary><b> selenium: 基础 </b></summary>

```

安装 selenium:
    1.首先需要下载Chromedriver,
        下载后得到的是一个chromedriver.exe文件(Google 搜索: chromedriver Download )
        
    2.将chromedriver.exe拷贝至
        谷歌浏览器目录(我的是 C:\Program Files (x86)\Google\Chrome\Application) 
        以及
        python根目录 (我的是:  C:\Users\SpringBoot\AppData\Local\Programs\Python\Python37 )

    3.将谷歌浏览器环境变量添加到 系统环境变量的 path (按下 win 键, 搜索'系统高级' , 点击环境变量...)
...
根据 id 取值(记得加上 try/except ):
driver.find_element_by_id("kw").send_keys("Python3的安装")

根据 class 取值:
try:
    list = browser.find_elements_by_class_name('AAAAAAAAAAAAAAAAA')
    list[0].click()
except:
    print("没找到相应的节点...")
...
<details>
<summary><b> selenium: 案例 </b></summary>
...
if __name__ == '__main__' :
    search_webs('https://www.baidu.com')
...
url = "https://sz.58.com/job/?key=Java&classpolicy=job_A&final=1&jump=1"
browser.get(url)

time.sleep(5)

Elements_node = browser.find_elements_by_class_name('job_name')    # 找节点
                browser.find_elements_by_class_name('job_name clearfix')  --> 这是错误的, 只写一个 class 即可

for index, item in enumerate(Elements_node):
...
<details>
<summary><b> selenium: 将web页面下拉到最底端 </b></summary>
